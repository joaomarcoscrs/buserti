...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development buserti npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop buserti
    docker rm buserti
    docker run --name buserti -d --env-file /home/ubuntu/buserti.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/buserti:/dkdata \
        buserti start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./buserti/
...
echo_green "Welcome to buserti's dev env"
