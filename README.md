...
```bash
dk bash  # starts bash inside "buserti" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and buserti.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `buserti` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The buserti container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv buserti -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database buserti owned by a user buserti.

On your remote machine, create a file ~/buserti.env:

```
DJANGO_DB_PASSWORD=<buserti's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=buserti
DJANGO_DB_USER=buserti
DJANGO_DEBUG=0
```
...
```
server {
    server_name  buserti.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/buserti/static;
...
(Replace "buserti.example.com" with your production domain)
